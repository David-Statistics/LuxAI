...
def take_step(u, target, m, allow_city):
    ...
    if u.pos.y > target.pos.y:
        if (u.pos.x, u.pos.y - 1) not in occ_loc:
            if m.get_cell(u.pos.x, u.pos.y - 1).citytile is None:
    ...
    if u.pos.x > target.pos.x:
        if (u.pos.x - 1, u.pos.y) not in occ_loc:
            if m.get_cell(u.pos.x - 1, u.pos.y).citytile is None:
...
def agent(observation, configuration):
    ...
    for unit in player.units:
        ...
        if unit.is_worker():
            ...
            elif cities_powered(player) or player.city_tile_count == 0:
            ...
            if type(target) != str and target is not None:
                ...
                allow_cities[unit.id] = allow_city
            else:
    ...
    logging.info(f"{[x.id for x in player.units]}")
    logging.info(f"{[id for id in UNIT_LOCATIONS.keys()]}")
    logging.info(f"{[id for id in TARGET_LOCS.keys()]}")
    logging.info(f"{[id for id in starting_locs.keys()]}")
    logging.info(f"{[id for id in allow_cities.keys()]}")
            ...
            if unit.can_act():
                if starting_locs[unit.id][0] == UNIT_LOCATIONS[unit.id][0] and starting_locs[unit.id][1] == UNIT_LOCATIONS[unit.id][1]:
                    if unit.id in TARGET_LOCS.keys():
                        logging.info(f"checking unit {unit.id}")
                        move_cmd = take_step(unit, TARGET_LOCS[unit.id], game_state.map, allow_cities[unit.id])
